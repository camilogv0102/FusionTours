...
if (!function_exists('mv_trip_base_price_for_selected_date')){
    function mv_trip_base_price_for_selected_date($product_id){
        if (!function_exists('mv_get_persona_price_matrix')) return 0.0;
        $matrix  = mv_get_persona_price_matrix($product_id);
        if (empty($matrix)) return 0.0;
        $adult_id = mv_find_adult_extra_for_product($product_id);
        if (!$adult_id || empty($matrix[$adult_id])) return 0.0;
        $location = function_exists('mv_get_active_tarifa_location') ? mv_get_active_tarifa_location($product_id) : 'playa_del_carmen';
        $prices   = $matrix[$adult_id];
        if (isset($prices[$location]) && $prices[$location] > 0) {
            return (float) $prices[$location];
        }
...
function mv_product_extra_price($product_id, $extra_id, $variant = null) {
    ...
    // Precios específicos para categoría Personas
    if (function_exists('mv_is_personas_extra') && mv_is_personas_extra($extra_id)) {
        if (function_exists('mv_get_persona_price_matrix')) {
            $matrix = mv_get_persona_price_matrix($product_id);
            if (isset($matrix[$extra_id]) && is_array($matrix[$extra_id])) {
                $location = function_exists('mv_get_active_tarifa_location') ? mv_get_active_tarifa_location($product_id) : 'playa_del_carmen';
                $row      = $matrix[$extra_id];
                if (isset($row[$location]) && $row[$location] > 0) {
    ...
    // Extras "Individual": intentar precio por índice legado
